<template>
    <div class="index-vue-buy">
        ...
        <div class="content-area">
            <Table ref="table" class="table" stripe :width="902" :columns="columns" :data="datas">
                ...
                <template slot-scope="{ row }" slot="content_a">
                    <Icon v-if="row.content_a" type="md-checkmark-circle-outline" class="icon-click" />
                </template>
                <template slot-scope="{ row }" slot="content_b">
                    <Icon v-if="row.content_b" type="md-checkmark-circle-outline" class="icon-click" />
                </template>
                <template slot-scope="{ row }" slot="content_c">
                    <Icon v-if="row.content_c" type="md-checkmark-circle-outline" class="icon-click" />
                </template>
            </Table>
...
import testImg from '../../../assets/bg03.jpg';
...
export default {
    ...
    beforeMount() {
            ...
            },
            {
                ...
                renderHeader: (h, params) => {
                    return h('div', {class: 'card-title'}, [
                        ...
                        h('Button', {
                            props: { type: 'primary', slot: 'content' },
                            on: { click: () => {this.showPay = true;} },
        ...
        this.datas = _.cloneDeep(this.menu);
        this.datas.forEach(item => {
            item.bought = false;
            item.detail = '说明: 介绍介绍介绍介绍介绍介绍介绍介绍介绍介绍介绍介绍';
            this.columns.forEach((val) => {
                if (val.key !== 'text') item[val.key] = true;
            });
        })
    }
}
